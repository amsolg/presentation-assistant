...
Test d'ajout de slide 17 (message centre Premier Tech)

Ce test verifie la capacite du script 04 a ajouter une slide de message
centre dans une presentation existante en utilisant le template slide 17.

Workflow du test:
1. Creer une presentation de base avec le script 01
2. Ajouter un message centre avec le script 04
...
class TestSlide17Centered:
    """
    Classe de test pour verifier l'ajout de slide 17 avec le script 04.
    """

    def __init__(self):
        self.test_name = "Test Unitaire - Script 04 - Slide 17"
    ...
    def create_base_presentation(self):
        ...
        try:
            ...
            base_subtitle = "Presentation de base pour test message centre"
            base_metadata = f"{datetime.now().strftime('%Y.%m.%d')} - Test Message Centre"
            base_project = "test_script_04_slide_17"
    ...
    def add_simple_message_slide(self, base_presentation_path):
        """
        Ajoute une slide de message centre avec le script 04.
        ...
        """
        try:
            print(f"MESSAGE Etape 2: Ajout du message centre...")

            # Parametres pour le message centre
            message_text = "Voici un exemple de message centre pour notre test unitaire"

            # Commande pour ajouter le message centre
            cmd_04 = [
                sys.executable,
            ...
            print(f"EXEC Ajout de message centre:")
            print(f"   {' '.join(cmd_04)}")
            ...
            # Verifier le resultat
            if result.returncode != 0:
                print(f"ERREUR Erreur lors de l'ajout de message centre:")
                print(f"   STDOUT: {result.stdout}")
                print(f"   STDERR: {result.stderr}")
                return False
            ...
            print(f"SUCCES Message centre ajoute avec succes!")
            ...
            print(f"   INFO Style: centered (slide 17)")

            return True

        except subprocess.TimeoutExpired:
            print(f"ERREUR Timeout lors de l'ajout de message centre")
            return False
        except Exception as e:
            print(f"ERREUR Erreur lors de l'ajout de message centre: {e}")
            return False
    ...
    def run_all_tests(self):
        """
        Execute tous les tests pour le script 04 slide 17.
        ...
        try:
            ...
            # Test 3: Ajout de message centre (seulement si la base est creee)
            ...
            print(f"MESSAGE Ajout message centre (script 04): {'REUSSI' if message_success else 'ECHEC'}")
...
def main():
    ...
    test_runner = TestSlide17Centered()
