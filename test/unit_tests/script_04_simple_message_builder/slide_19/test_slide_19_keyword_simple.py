#!/usr/bin/env python3
"""
Test unitaire pour le script 04 - Simple Message Builder
Test d'ajout de slide 19 (message avec keywords simple Premier Tech)

Ce test verifie la capacite du script 04 a ajouter une slide de message
avec keywords simple dans une presentation existante en utilisant le template slide 19.

Workflow du test:
1. Creer une presentation de base avec le script 01
2. Ajouter un message avec keywords simple avec le script 04
...
class TestSlide19KeywordSimple:
    """
    Classe de test pour verifier l'ajout de slide 19 avec le script 04.
    """

    def __init__(self):
        self.test_name = "Test Unitaire - Script 04 - Slide 19"
        self.script_01_path = Path(__file__).parent.parent.parent.parent.parent / "presentation_builder" / "01_slide_title_creator.py"
        self.script_04_path = Path(__file__).parent.parent.parent.parent.parent / "presentation_builder" / "04_simple_message_builder.py"
    ...
    def setup_test(self):
        ...
        print(f"SCRIPT Script 04 (teste): {self.script_04_path}")
        ...
        if not self.script_04_path.exists():
            raise FileNotFoundError(f"Script 04 non trouve: {self.script_04_path}")
    ...
    def create_base_presentation(self):
        ...
        try:
            ...
            base_title = "TEST UNITAIRE - Script 04"
            base_subtitle = "Presentation de base pour test message keywords simple"
            base_metadata = f"{datetime.now().strftime('%Y.%m.%d')} - Test Keywords Simple"
            base_project = "test_script_04_slide_19"
    ...
    def add_simple_message_slide(self, base_presentation_path):
        """
        Ajoute une slide de message avec keywords simple avec le script 04.
        ...
        """
        try:
            print(f"MESSAGE Etape 2: Ajout du message avec keywords simple...")

            # Parametres pour le message avec keywords simple
            message_text = "Innovation et Excellence au coeur de notre strategie"

            # Commande pour ajouter le message avec keywords simple
            cmd_04 = [
                sys.executable,
                str(self.script_04_path),
                message_text,
                "--style", "keyword_simple",
                "--keywords", "Innovation, Technologie, Excellence",
                "--insert-into", str(base_presentation_path)
            ]

            print(f"EXEC Ajout de message keywords simple:")
            print(f"   {' '.join(cmd_04)}")

            # Executer le script 04 depuis la racine du projet
            project_root = self.script_04_path.parent.parent
            result = subprocess.run(
                cmd_04,
                cwd=project_root,
                capture_output=True,
            ...
            # Verifier le resultat
            if result.returncode != 0:
                print(f"ERREUR Erreur lors de l'ajout de message keywords simple:")
                print(f"   STDOUT: {result.stdout}")
                print(f"   STDERR: {result.stderr}")
                return False
            ...
            print(f"SUCCES Message keywords simple ajoute avec succes!")
            ...
            print(f"   INFO Message: {message_text}")
            print(f"   INFO Style: keyword_simple (slide 19)")
            print(f"   INFO Keywords: Innovation, Technologie, Excellence")

            return True

        except subprocess.TimeoutExpired:
            print(f"ERREUR Timeout lors de l'ajout de message keywords simple")
            return False
        except Exception as e:
            print(f"ERREUR Erreur lors de l'ajout de message keywords simple: {e}")
            return False

    def run_validation_test(self):
        """
        Execute le test de validation du script 04.

        Returns:
            bool: True si la validation reussit, False sinon
        """
        try:
            print(f"TEST Test de validation du script 04...")

            # Commande de validation (avec message factice requis)
            cmd = [
                sys.executable,
                str(self.script_04_path),
                "dummy",
            ...
            print(f"   RESULTAT Script 04 valide avec succes")
    ...
    def run_all_tests(self):
        """
        Execute tous les tests pour le script 04 slide 19.
        ...
        try:
            ...
            # Test 3: Ajout de message keywords simple (seulement si la base est creee)
            message_success = False
            if base_creation_success:
                message_success = self.add_simple_message_slide(base_presentation_path)

            self.cleanup_test()

            # Resultat final
            all_success = validation_success and base_creation_success and message_success
            ...
            print(f"MESSAGE Ajout message keywords simple (script 04): {'REUSSI' if message_success else 'ECHEC'}")
...
def main():
    ...
    test_runner = TestSlide19KeywordSimple()
