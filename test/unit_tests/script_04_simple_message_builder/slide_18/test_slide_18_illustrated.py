...
Test d'ajout de slide 18 (message illustre Premier Tech)

Ce test verifie la capacite du script 04 a ajouter une slide de message
illustre dans une presentation existante en utilisant le template slide 18.

Workflow du test:
1. Creer une presentation de base avec le script 01
2. Ajouter un message illustre avec le script 04
...
class TestSlide18Illustrated:
    """
    Classe de test pour verifier l'ajout de slide 18 avec le script 04.
    """

    def __init__(self):
        self.test_name = "Test Unitaire - Script 04 - Slide 18"
    ...
    def create_base_presentation(self):
        ...
        try:
            ...
            base_subtitle = "Presentation de base pour test message illustre"
            base_metadata = f"{datetime.now().strftime('%Y.%m.%d')} - Test Message Illustre"
            base_project = "test_script_04_slide_18"
    ...
    def add_simple_message_slide(self, base_presentation_path):
        """
        Ajoute une slide de message illustre avec le script 04.
        ...
        """
        try:
            print(f"MESSAGE Etape 2: Ajout du message illustre...")

            # Parametres pour le message illustre
            message_text = "Voici un exemple de message illustre avec image pour notre test unitaire"

            # Commande pour ajouter le message illustre
            cmd_04 = [
                sys.executable,
                str(self.script_04_path),
                message_text,
                "--style", "illustrated",
                "--image-description", "Image representative du concept presente",
                "--insert-into", str(base_presentation_path)
            ]

            print(f"EXEC Ajout de message illustre:")
            print(f"   {' '.join(cmd_04)}")
            ...
            # Verifier le resultat
            if result.returncode != 0:
                print(f"ERREUR Erreur lors de l'ajout de message illustre:")
                print(f"   STDOUT: {result.stdout}")
                print(f"   STDERR: {result.stderr}")
                return False
            ...
            print(f"SUCCES Message illustre ajoute avec succes!")
            ...
            print(f"   INFO Style: illustrated (slide 18)")

            return True

        except subprocess.TimeoutExpired:
            print(f"ERREUR Timeout lors de l'ajout de message illustre")
            return False
        except Exception as e:
            print(f"ERREUR Erreur lors de l'ajout de message illustre: {e}")
            return False
    ...
    def run_all_tests(self):
        """
        Execute tous les tests pour le script 04 slide 18.
        ...
        try:
            ...
            # Test 3: Ajout de message illustre (seulement si la base est creee)
            ...
            print(f"MESSAGE Ajout message illustre (script 04): {'REUSSI' if message_success else 'ECHEC'}")
...
def main():
    ...
    test_runner = TestSlide18Illustrated()
